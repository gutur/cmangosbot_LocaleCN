...
namespace ai
{
    ...
    {
    public:
        MyThreatValue(PlayerbotAI* ai, string name = "my threat") : LogCalculatedValue(ai, name), Qualified() { minChangeInterval = 1; lastTarget = ObjectGuid(); }
    ...
    {
    public:
        TankThreatValue(PlayerbotAI* ai, string name = "tank threat") : FloatCalculatedValue(ai, name), Qualified() {}
        virtual float Calculate();
    };
	...
	{
	public:
        ThreatValue(PlayerbotAI* ai, string name = "threat") : Uint8CalculatedValue(ai, name), Qualified() {}
